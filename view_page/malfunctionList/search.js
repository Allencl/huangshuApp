...
import { TouchableOpacity,TextInput,DeviceEventEmitter,Dimensions,StyleSheet, ScrollView, View,Text,NativeModules   } from 'react-native';
...
class Page extends Component {
  ...
  componentDidMount(){
    ...
    this.honeyWellPrint=DeviceEventEmitter.addListener('globalEmitter_honeyWell',function(key=""){
      ...
      let _newKey=_key.split("-");
      let _id=Number(_newKey[_newKey["length"]-1]);

      // console.log(_id)
      // ToastExample.show( JSON.stringify(_newKey[_newKey["length"]-1][0]) );
      // ToastExample.show( 123 );
      // Toast.success(  _id );
  ...
  searchData=(_id)=>{
    ...
    WISHttpUtils.post("tmBatchingTask/getBathingTaskInfo.do",{
      ...
      that.setState({
        dataList1:[(data.base||{})],
        dataList2:[(data.base||{})],
        dataList3:(data.detailList||[]).map((o,i)=>Object.assign(o,{_index:i+1}))
      });
    }); 
